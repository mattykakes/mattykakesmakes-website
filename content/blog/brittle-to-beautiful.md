...
This is easy enough when dealing with class members -- it's _Programming 101_. What this looks like with UI components can be much less straightforward. What do you do at a level above members and methods?
...
Now, let's say the project doesn’t have a strong encapsulation precedent, or even a step further, for example's sake, and say the project doesn’t prevent the use of child components elsewhere.
...
The PicklistRow already has the item data needed to calculate a user’s total and maps image resources appropriately. It might have too much functionality built into it but it has what you need - what could it hurt?
...
* In the dashboard module and exported alongside it to share the local dashboardService? 
...
Directory prefixes within a large directory like `features/` help us infer how a module is used. See that both the codes list and codes tab are feature components. We can infer that the `tab-codes/` directory is more of a top level component while the `feature-codes-list/` is intended for modular reuse.
...
However, streams can become tangled when data flows aren’t clearly scoped or documented. Without a disciplined structure, the reactive model meant to simplify state propagation can instead lead to a [rat’s nest](https://englishteststore.net/lesson/english-idioms/rats-nest-idiom-meaning-and-example-usage-in-sentences/) of observables — and if you’ve ever worked on a reactive project where tracing state feels like deciphering this state diagram, you know exactly how frustrating that can be.
...
Poor separation of concerns, otherwise known as the [single responsibility principle](https://en.wikipedia.org/wiki/Single-responsibility_principle), is where objects do not adhere to functional boundaries. Oftentimes this occurs when components contain subject sources, or business logic, that instead belong encapsulated within a dedicated service or utility. A lack of logical structure makes the code harder to understand, maintain, and test, as changes in one area can unintentionally affect unrelated parts of the application.

For a reactive programming project to maintain a high level of robustness over time, the above issues must be mitigated. A strict state management approach needs to be defined and adhered to. We can achieve this by breaking down state management into two separate categories: local and global states.
...
Now imagine the issue compounds -- component 2 is part of a larger network of components with a shared global state. Let's say component 2 observes another state from the global store, and its action depends on the states received from both the local service and the global store. Now the referenced object changes asynchronously through a more complex reactive programming model.
...
Inconsistent formatting also hurts readability. Enterprise applications rarely belong to a single person, which means no one should be free to format code according to their personal preference. Oftentimes, it’s hard to avoid without a safeguard. Teams, or even developers within a team, use different tools. How often do developers have different IDE configurations within the same tool ecosystem? This alone can lead to inconsistencies.
...
There are many ways to do this. It’s common to make linting a part of the [CI](https://en.wikipedia.org/wiki/Continuous_integration) pipeline -- where linting must succeed before a package can be built for a release or a branch merged into the main working branch. However, it's easy for a busy team to overlook failing builds during a lull in the release schedule, leaving cleanup to the unlucky release manager who discovers it.
...
Without a written plan, a project can have the best design in the world at its inception -- but if it is intended to be long-lived and involve multiple people, it will eventually fall into chaos.
